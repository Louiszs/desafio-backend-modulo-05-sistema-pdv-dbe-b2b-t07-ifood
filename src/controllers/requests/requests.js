...
const requestRegister = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < pedido_produtos.length; i++) {
      ...
      const stock = productExists[0].quantidade_stock;
    ...
    for (let i = 0; i < pedido_produtos.length; i++) {
      ...
      const solicitedQuanty = pedido_produtos[i].quantidade_produto;
      const produtcPrice = productExists[0].valor;
      const attStock = productExists[0].quantidade_stock - solicitedQuanty;

      await knex("produtos")
        .where({ id: productId })
        .update({ quantidade_stock: attStock });
    ...
    const { nome, email } = clientExists[0];
